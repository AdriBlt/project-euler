...
namespace Euler.Problems
{
    ...
    public class P011 : AbstractProblem
    {
        ...
        public override void DebugProblem()
        {
            ...
            DebugResult(prod, 1788696);
        }

        public override long ComputeSolution()
        {
            return GetGreatestProduct(Matrix);
        }
        /**
         * @param args
         */
        public long GetGreatestProduct(int[][] matrix)
        {
            ...
            int prodSize = 4;
            long maxProd = 0;
            for (int i = 0; i <= matrixSize - prodSize; i++)
            {
                for (int j = 0; j < matrixSize; j++)
                {
                    ...
                    {
                        col = col * matrix[i + k][j];
                        lin = lin * matrix[j][i + k];
                    }
                ...
                for (int j = 0; j <= matrixSize - prodSize; j++)
                {
                    int diag = 1;
                    int cdiag = 1;
                    for (int k = 0; k < prodSize; k++)
                    {
                        diag = diag * matrix[i + k][j + k];
                        cdiag = cdiag * matrix[i + k][j + prodSize - k - 1];
                    }
